...
> ## ⚠ 「単一遺伝子は最適」という主張への反証（2026-08-02）
>
> 本文 §1 は補完エピトープ（core fucose / bisecting / β1,6分岐）について
> 「**単一遺伝子にクリーンに対応（＝薬剤誘導potentialのスコアリングにむしろ最適）**」と書いている。
> **クリーンさの意味では正しいが、手法の実証という観点では逆に働く。**
>
> 単一遺伝子エピトープでは GlycoMaple ロジック（内max × 間min）が**恒等写像**になる。
> つまり論文が方法論的貢献として掲げる「素朴な和でなく生合成経路を反映したスコア」が、
> **これらのエピトープでは一切テストされない。** wet が示すのは
> 「FUT8 の mRNA が上がると AAL 反応性が上がる」——正しいが教科書的である。
>
> さらに実データでは、**HepG2 で core fucose を2倍動かす薬は175本中2本しかない**
> （β1,6分岐は22本）。§3.1 の「最優先」「HCC論旨の要」という位置づけはデータに支持されない。
> 詳細と代替案は `docs/wet_target_selection.md`。
>
> ※ 補完という**方針自体**（元DBが終末決定基中心で構造クラスを収載しない、という系統的な穴を埋める）は
> 有効なので、この文書は破棄しない。効いていないのは「単一遺伝子＝最適」という評価と、
> core fucose を筆頭に置く優先順位だけである。
