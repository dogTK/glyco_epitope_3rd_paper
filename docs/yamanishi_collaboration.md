...
> **補足（2026-08-02）**：山西先生は著者の**指導教員**でもある（論文博士）。
> 体制は実際に共同研究で、**dry は著者（在野研究者・実験環境なし）、wet は山西研側に依頼する**。
> したがって下の「分担イメージ」はおおむね妥当。ただし wet は依頼である以上、
> **dry が「試す価値がある」と思わせる出来である必要がある**（依頼の設計は
> `docs/roadmap_to_thesis.md` Phase 2）。
> ※ 一時この欄に「共同研究ではない」と書いていたが、それは誤りだった。
