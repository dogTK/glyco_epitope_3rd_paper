...
> **山西芳裕先生はユーザーの指導教員であり、山西研（九大→名大）は所属研究室である**（外部の共同研究相手ではない。2026-08-02訂正）。
> したがって山西研の論文群は「競合の先行研究」ではなく**自分の研究室の資産**として扱う。
> 手法は house style（細胞内マッチング＝Iwata 2017、経路スコア＋wet検証＝Iwata 2018）に素直に乗せてよく、
> むしろ乗る方が博論では加点になる。問いは「どう差別化するか」ではなく
> **「研究室の蓄積に、自分が何を足すか」**（＝糖鎖の実測層。`docs/roadmap_to_thesis.md`）。

「既にやられていないか」「自分の寄与がどこか」を毎回そこで確認する。書誌一覧は `inputs/papers/README.md`。

**Akiyoshi et al., Mol Inf 2020**（山西研）は、CREEDS疾患シグネチャ×glycogeneで
**疾患-疾患の糖鎖類似度マップ（PCA/GTM）を既に発表済み**。これを踏まえて発展させるのは正しく、
引用して延長する形が望ましい。避けるのは追試になることだけ——
「AAとBBが糖鎖的に近い」を**結論でなく前提**として引用し、その先の**転用可能性**
（共有エピトープを動かす薬／検出レクチン・抗体がAAからBBへ移せるか）を出す。
差別化3軸は「薬剤軸」「エピトープ解像度＋方向性」「認識分子/標的性」。
